...
// Three.js is dynamically imported and the whole particle system is deferred
// until the browser is idle (post-load). This keeps the 597 kB three chunk and
// the 3800-point init off the critical path — they no longer block TBT. The
// loader covers the hero while this spins up, so nothing is visibly missing.
export default function HeroParticles() {
  ...
  useEffect(() => {
    ...
    let cancelled = false
    let renderer = null, animId = 0, intervalId = 0
    let onMouse = null, onResize = null
    ...
    // Defer to an idle period after load so the heavy three eval + particle
    // build never lands inside the interactive window.
    const ric = window.requestIdleCallback
    const handle = ric ? ric(start, { timeout: 2500 }) : setTimeout(start, 800)
